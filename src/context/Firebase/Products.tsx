...
import { Action, initialState, productsReducer } from '../../reducers/productsReducer';
...
export const ProductsContextProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const productsDataFetcher=async()=>{
        if(firebase?.data?.user?.email){
            try {
                ...
                const productsData: any[] = await Promise.all(productsSnapshot?.docs?.map(async (doc: any) => {
    ...
    return <ProductsContext.Provider value={{ data, dispatch,productsDataFetcher }}>{children}</ProductsContext.Provider>;
};
